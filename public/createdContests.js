...
function timerFunction(startTime,duration,timer,start,contest,id){
    let timerFunctionality =setInterval(()=>{
        ...
        if(currentTime>=endTime){
            ...
            resultsLink.setAttribute("href","http://localhost:2000/generateresults/"+id)
...
fetch("http://localhost:2000/createdContests",{
    "method":"post",
    "headers":{
    ...
    data.forEach((doc)=>{
        ...
        function startHandler(){
            fetch("http://localhost:2000/startContest",{
                "method":"post",
                headers :{
                    "Content-Type":"application/json"
                },
                ...
                if(data == "ok"){
                    window.location.replace("http://localhost:2000/mycontests")
                }
            })                
        }
        ...
        for(let question in doc.questions){
            ...
            questionLink.innerText = question
        ...
        if(startTime==null){
            ...
            if(doc.reports == null){
                resultsLink.innerText = "generate results (a new tab will be opened dont close it till you get results)"
                resultsLink.setAttribute("href","http://localhost:2000/generateresults/"+contest.contestID)
            }
            else{
                resultsLink.innerText = "results"
                resultsLink.setAttribute("href","http://localhost:2000/results/"+contest.contestID)
            } 
