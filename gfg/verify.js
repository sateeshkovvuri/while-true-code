...
const verify = (submission_id,secretcode) =>{
    return new Promise ((accept,reject)=>{
        try{
            const url = `https://practiceapi.geeksforgeeks.org/api/latest/problems/submissions/${submission_id}/`
            fetch (url,{
                "headers":{
                    "Cookie":cookie
                }
                ...
                if(data.user_code.indexOf(secretcode)!=-1){
                    accept("verified")
                }
                else{
                    accept("not verfied")
